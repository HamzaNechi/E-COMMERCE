      ...
      <form method="get" action="{{ url('/search_Prod')}}">
        <div class="form-group">
          <div class="input-group">
            <input type="text" class="form-control form-control-sm" placeholder="Chercher un produit" aria-label="Recipient's username" aria-describedby="basic-addon2" name="recherche">
              ...
              <div class="col-12 grid-margin">
                <div class="card">
                  <div class="card-body">
                    ...
                    <div class="table-responsive">
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          <tr>
                          <td>
                            <img src="img/produit/s/{{ $row->image}}">
                          </td>
                          ...
                          <a href="{{url('/Admin/DetailProduit',$row->id)}}">
                          <button type="button" class="btn btn-warning btn-rounded btn-icon" title="Voir détail">
                        ...
                        <!--afficher les pack-->
                        @if($pack != NULL && $product==NULL)
                        <tbody>
                          @foreach($pack as $row)
                          <tr>
                          <td>
                            <img src='{{URL::asset("img/produit/s/$row->image")}}'>
                          </td>
                          ...
                          <a href="{{url('/Pack/Detail_Pack',$row->id)}}">
                          <button type="button" class="btn btn-warning btn-rounded btn-icon" title="Voir détail">
                            <i class="mdi mdi-eye"></i>
                          </button></a>
                    ...
                    @if($pack == NULL && $product != NULL)
                    {{ $product->links('vendor.pagination.dashboard-paginator') }}
                    @else
                    {{ $pack->links('vendor.pagination.dashboard-paginator') }}
                    @endif
                  </div>
                </div>
              </div>
