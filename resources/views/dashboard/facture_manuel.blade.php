...
<div class="container-fluid invoice-container"> 
  ...
  <header>
    <div class="row align-items-center">
      ...
      <div class="col-sm-5 text-center text-sm-right">
        ...
        <p class="mb-0">Numéro de facture - {{ $facture->id }}</p>
  ...
  <main>
    <div class="row">
      <div class="col-sm-6 text-sm-right order-sm-1">
        ...
        <address>
        {{ $facture->client }}<br />
        {{ $facture->region}} , {{ $facture->ville }}<br />
        {{ $facture->adresse}}<br>
        {{ $facture->tel }}<br>
        {{ $facture->postal }}
        </address>
        ...
        <td class="col-2 text-center border-top-0"><strong>Prix</strong></td>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><span class="text-3">{{ $key->designation }}</span> </td>
          <td class="text-center">{{ $key->qty }}</td>
                <td class="text-center">{{ $key->prix_venteHT }}</td>
                
                <td class="text-right">{{ $key->montant_ttc }}</td>
              </tr>
  ...
  <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
              ...
              <tr>
                <td colspan="2" class="bg-light-2 text-right"><strong>Total HT</strong></td>
                <td colspan="2" class="bg-light-2 text-right">{{ $facture->total_ht }} TND</td>
              </tr>


              @if($facture->promo > 0)
                <tr>
                <td colspan="2" class="bg-light-2 text-right"><strong>Promotion</strong><br>
                  @if($facture->code != NULL && $facture->code != "/" )
                  <span class="text-1">{{ $facture->code }}</span>
                  @endif
                </td>
                <td colspan="2" class="bg-light-2 text-right">- {{ $facture->promo }} TND</td>
                
              </tr>
              @endif
              @if($facture->tva > 0)
              <tr>
                <td colspan="2" class="bg-light-2 text-right"><strong>TVA</strong><br>
                  <span class="text-1">
                  <?php $tot=$facture->total_ht*($facture->tva/100); ?>
                  <?php echo $tot ; ?> TND</span>
                </td>
                <td colspan="2" class="bg-light-2 text-right">+ {{ $facture->tva }} %</td>
              ...
              <tr>
                ...
                <td colspan="2" class="bg-light-2 text-right">{{ $facture->total_ttc }} TND</td>
              </tr>
              @if($facture->methode=="Par tranche" && $facture->net >0)
                <tr>
                <td colspan="2" class="bg-light-2 text-right"><strong>Montant payé</strong><br>
                  
                </td>
                <td colspan="2" class="bg-light-2 text-right">+ {{ $facture->tranche }}
                </td>
              </tr>
              
              <tr>
                <td colspan="2" class="bg-light-2 text-right"><strong>Net à payé</strong></td>
                <td colspan="2" class="bg-light-2 text-right">{{ $facture->net }} TND</td>
              </tr>
              @endif
